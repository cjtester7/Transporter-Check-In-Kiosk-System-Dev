...
// Version: appscript-v23.gs
// Deploy as Web App: Execute as Me, Anyone can access

// ============================================================
// CONFIGURATION
const SHEET_ID = "1fAcPohNPc32egYUeLq3SbZGZnEufQNrggM0xTpn9a2w";
...
// COL is built dynamically from the actual sheet header row at runtime
// This ensures column indexes are always correct regardless of sheet column order
const COL = {};
function buildCOL(sheet) {
  const headerRow = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  headerRow.forEach((h, i) => { if (h) COL[h] = i; });
}
...
function doGet(e) {
  ...
  try {
    ...
    let result;
    switch (action) {
      case "ping":           result = { pong: true, version: "v23", openById: true, timestamp: new Date().toISOString() }; break;
...
function getSheet() {
  ...
  buildCOL(sheet); // always sync COL to actual sheet column order
  return sheet;
}
...
function getAllRecords() {
  ...
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return { records: [], _v: 23 };
  const headers = data[0];
  const tz = Session.getScriptTimeZone();
