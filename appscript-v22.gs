...
function doGet(e) {
  ...
  try {
    ...
    let result;
    switch (action) {
      case "ping":           result = { pong: true, version: "v22", openById: true, timestamp: new Date().toISOString() }; break;
...
function getAllRecords() {
  ...
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return { records: [], _v: 22 };
  const headers = data[0];
  const tz = Session.getScriptTimeZone();
