...
function doGet(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "reorderQueue":   result = reorderQueue(body); break;
...
function reorderQueue(data) {
  const sheet = getSheet();
  const allData = sheet.getDataRange().getValues();
  const rowId = data.rowId;
  const newPos = parseInt(data.newPosition);
  if (!rowId || isNaN(newPos) || newPos < 1) return { error: "Invalid row ID or position." };

  // Get all active records sorted by current timestamp
  const activeRows = [];
  for (let i = 1; i < allData.length; i++) {
    ...
    if (status === "Waiting" || status === "In Progress") {
      activeRows.push({
        rowNum: i + 1,
        rowId: allData[i][COL["Row ID"]],
        ts: Number(allData[i][COL["Check-In Timestamp"]]) || 0
      });
    }
  }
  ...
  const targetIdx = activeRows.findIndex(r => r.rowId == rowId);
  if (targetIdx === -1) return { error: "Record not found in active queue." };

  // Clamp new position to valid range
  const clampedPos = Math.max(1, Math.min(newPos, activeRows.length));

  // Reorder — remove from current position, insert at new position
  const [moved] = activeRows.splice(targetIdx, 1);
  activeRows.splice(clampedPos - 1, 0, moved);

  // Write new queue positions and timestamps to sheet
  // Adjust timestamps to preserve order without changing check-in times
  ...
  // Update Check-In Timestamp of moved record so resequenceQueue preserves this order
  // Use a synthetic timestamp that slots it into the correct position
  if (clampedPos > 1 && clampedPos <= activeRows.length) {
    const prevTs = Number(allData.find((r, i) => i > 0 && allData[i][COL["Row ID"]] == activeRows[clampedPos - 2].rowId)?.[COL["Check-In Timestamp"]]) || 0;
    const nextTs = Number(allData.find((r, i) => i > 0 && allData[i][COL["Row ID"]] == activeRows[clampedPos].rowId)?.[COL["Check-In Timestamp"]]) || 0;
    if (prevTs > 0 && nextTs > 0) {
      const midTs = Math.floor((prevTs + nextTs) / 2);
      for (let i = 1; i < allData.length; i++) {
        if (allData[i][COL["Row ID"]] == rowId) {
          sheet.getRange(i + 1, COL["Check-In Timestamp"] + 1).setValue(midTs);
          break;
        }
      }
    }
  ...
  return { success: true, newPosition: clampedPos, total: activeRows.length };
}
